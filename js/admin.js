...
const els = {
  list: $('#adminList'), count: $('#adminCount'), dirty: $('#adminDirty'), search: $('#adminSearch'),
  form: $('#editorForm'), empty: $('#emptyEditor'), title: $('#editorTitle'),
  importButton: $('#importButton'), importFile: $('#importFile'), exportButton: $('#exportButton'),
  addButton: $('#addButton'), resetButton: $('#resetButton'), deleteButton: $('#deleteButton'),
  hint: $('#editorHint'),
...
const defaults = {
  platform: 'Twitch', role: 'Moderator', status: 'needs-confirmation', displayName: '',
  started: '', ended: '', endReason: '', description: '', avatar: '', services: []
};
...
function loadData() {
  try {
    ...
    console.warn('[Lew3l1 Admin] local draft unavailable', error);
  }
  ...
  fetch(DATA_URL, { cache: 'no-store' })
    ...
    .catch((error) => {
      ...
      state.items = [];
      renderList();
      setMessage('Не удалось загрузить data/streamers.json. Импортируй JSON вручную.');
    });
}
...
function statusLabel(item) {
  return ({
    active: 'ACTIVE',
    former_self: 'FORMER · свои причины',
    former_inactive: 'FORMER · инактив',
    former: 'FORMER',
    'needs-confirmation': 'VERIFY'
  })[item.status] || 'VERIFY';
}

function renderList() {
  const query = els.search.value.trim().toLowerCase();
  const items = state.items.map((item, index) => ({ item, index })).filter(({ item }) => {
    return `${item.username} ${item.displayName}`.toLowerCase().includes(query);
  });
  ...
  els.list.innerHTML = items.length ? items.map(({ item, index }) => `
    <button class="admin-list-item ${state.selected === index ? 'is-selected' : ''}" type="button" data-index="${index}">
      <span class="admin-avatar">${escapeHtml((item.username || '??').slice(0, 2).toUpperCase())}</span>
      ...
      <span class="admin-item-status status-${escapeHtml(item.status || 'needs-confirmation')}">${statusLabel(item)}</span>
...
function select(index) {
  ...
  els.displayName.value = item.displayName;
  els.year.value = item.year;
  els.started.value = item.started;
  els.role.value = item.role;
  els.platform.value = item.platform;
  els.status.value = item.status;
  els.ended.value = item.ended;
  els.endReason.value = item.endReason;
  els.description.value = item.description;
  els.avatar.value = item.avatar;
...
function readForm() {
  return normalize({
    ...
    year: els.year.value,
    started: els.started.value,
    role: els.role.value,
    platform: els.platform.value,
    status: els.status.value,
    ended: els.ended.value,
    endReason: els.endReason.value,
...
function writeCurrent() {
  ...
  const item = readForm();
  if (!item.username || !/^\d{4}$/.test(item.year)) {
    setMessage('Username и корректный год обязательны.');
    return false;
  }
...
function addItem() {
  writeCurrent();
  const item = normalize({ username: `new-channel-${state.items.length + 1}`, year: String(new Date().getFullYear()), status: 'needs-confirmation' });
...
function deleteItem() {
  ...
  if (!window.confirm(`Удалить @${item.username}? Это изменит только локальный черновик.`)) return;
...
function resetDraft() {
  ...
  fetch(DATA_URL, { cache: 'no-store' })
    ...
    .then((items) => { state.items = items.map(normalize); renderList(); setMessage('Черновик сброшен.'); })
    .catch(() => setMessage('Не удалось восстановить исходный JSON.'));
}
...
function importJson(file) {
  ...
  reader.onload = () => {
    try {
      ...
      state.dirty = new Set(state.items.map((_, index) => index));
      state.selected = -1;
      saveDraft();
      els.form.hidden = true;
      els.empty.hidden = false;
      setMessage(`Импортировано ${state.items.length} записей.`);
    } catch (error) {
      setMessage('Ошибка: файл не является корректным массивом JSON.');
    }
  };
...
['input', 'change'].forEach((eventName) => {
  els.form.addEventListener(eventName, () => {
    if (state.selected < 0) return;
    state.items[state.selected] = readForm();
    markDirty(state.selected);
    els.title.textContent = `@${state.items[state.selected].username || 'new-channel'}`;
  });
});
